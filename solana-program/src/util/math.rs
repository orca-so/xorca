...
pub fn convert_orca_to_xorca(
    ...
    // Add virtual assets to the xORCA supply and non-escrowed ORCA amount to disincentivize vault inflation attack.
    // (See https://docs.openzeppelin.com/contracts/4.x/erc4626#defending_with_a_virtual_offset)
    let xorca_supply_with_virtual_amount_u128 = xorca_supply_u128
        .checked_add(VIRTUAL_XORCA_SUPPLY)
        .ok_or(ErrorCode::ArithmeticError)?;
    let non_escrowed_orca_amount_with_virtual_amount_u128 = non_escrowed_orca_amount_u128
        .checked_add(VIRTUAL_NON_ESCROWED_ORCA_AMOUNT)
        .ok_or(ErrorCode::ArithmeticError)?;

    let out_xorca_amount_u128 = orca_amount_to_convert_u128
        .checked_mul(xorca_supply_with_virtual_amount_u128)
        .ok_or(ErrorCode::ArithmeticError)?
        .checked_div(non_escrowed_orca_amount_with_virtual_amount_u128)
        .ok_or(ErrorCode::ArithmeticError)?;
...
pub fn convert_xorca_to_orca(
    ...
    let out_orca_amount_u128 = xorca_amount_to_convert_u128
        .checked_mul(non_escrowed_orca_amount_with_virtual_amount_u128)
        .ok_or(ErrorCode::ArithmeticError)?
        .checked_div(xorca_supply_with_virtual_amount_u128)
        .ok_or(ErrorCode::ArithmeticError)?;
